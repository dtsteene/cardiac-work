#!/usr/bin/env bash
# Frank-Starling sweep on the shared h=5 UKB mesh, with shared unloading.
#
# Holds constant across all 16 cases:
#   * Mesh: data/mesh_convergence/ukb_L5/ukb/geometry  (CHAR_LENGTH = 5.0)
#   * Region mask: the same geometry_fields.npz feeds postprocessing for every case
#   * Unloaded reference: every case loads the same prestress_inverse.bp
#     (per-case inverse unloading is skipped via LOAD_UNLOADED_FROM)
#   * Active stress model: uniform scalar Ta + pulse.FrankStarlingActiveStress
#     (length-dependent activation, baked into complete_cycle.py)
#   * Postprocessing: strict canonical mode (RUN_POSTPROCESS=1, POST_FULL=0)
#
# Only thing that varies: the per-case 0D circulation parameter JSON.
...
# Default shared unloaded reference: sPAP30 from the capped production sweep.
# Unloading runs with Ta = 0 throughout, so the shape is independent of which
# active model (plain ActiveStress vs FrankStarlingActiveStress) is configured.
...
RESULTS_ROOT="${RESULTS_ROOT:-${WORK_DIR}/results/sims/$(date +%F)/frankstarling_l5_${STAMP}}"
ANALYSIS_DIR="${ANALYSIS_DIR:-${WORK_DIR}/results/analysis/frankstarling_l5_sweep_${STAMP}}"
MANIFEST="${MANIFEST:-${ANALYSIS_DIR}/frankstarling_l5_cases.tsv}"
DRY_RUN="${DRY_RUN:-0}"
...
echo "============================================================"
echo "  Frank-Starling sweep on shared-unloaded h=5 UKB mesh"
echo "  Started:           $(date)"
echo "  Cases:             ${CASES}"
...
echo "  Active model:      FrankStarlingActiveStress (uniform scalar Ta)"
...
for case_name in ${CASES}; do
    ...
    comment="frankstarling_l5_${case_name}_h5_6bt_fixed_unloaded_reference"
    ...
    if [ -n "${jid}" ]; then
        JOB_IDS+=("${jid}")
        printf "%s\t%s\tfrankstarling_l5_submission_%s\tshared L5 geometry, shared unloaded reference, Frank-Starling active stress, strict canonical postprocessing; %s\t%s\t%s\t%s\t%s\t%s\n" \
