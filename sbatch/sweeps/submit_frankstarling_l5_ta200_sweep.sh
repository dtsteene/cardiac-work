#!/usr/bin/env bash
# Frank-Starling sweep, 8 cases, with Ta_peak scaled up 100 → 200 kPa.
#
# Rationale: with the live g(λ) Frank-Starling multiplier, the cycle-average
# multiplier is ~0.5 in our operating stretch range. Imposing the original
# Ta_peak=100 kPa therefore delivers only ~50 kPa effective active stress,
# which compresses the achievable cavity-pressure range. Setting Ta_peak=200
# kPa compensates the average suppression and restores the pressure range
# without changing the Frank-Starling implementation.
#
# Everything else matches submit_frankstarling_l5_sweep.sh:
#   - shared L5 UKB mesh + shared inverse-unloaded reference
#   - uniform scalar Ta
#   - live (instantaneous) Frank-Starling multiplier
#   - strict canonical postprocessing
...
# 8-case spread chosen to cover the full RV-pressure range
CASES="${CASES:-sPAP22 sPAP35 sPAP45 sPAP55 sPAP65 sPAP75 sPAP85 sPAP95}"
STAMP="${STAMP:-$(date +%Y%m%d_%H%M%S)}"
RESULTS_ROOT="${RESULTS_ROOT:-${WORK_DIR}/results/sims/$(date +%F)/frankstarling_l5_ta200_${STAMP}}"
ANALYSIS_DIR="${ANALYSIS_DIR:-${WORK_DIR}/results/analysis/frankstarling_l5_ta200_sweep_${STAMP}}"
MANIFEST="${MANIFEST:-${ANALYSIS_DIR}/frankstarling_l5_ta200_cases.tsv}"
DRY_RUN="${DRY_RUN:-0}"
...
echo "============================================================"
echo "  Frank-Starling sweep with Ta_peak = 200 kPa (compensation fix)"
echo "  Started:           $(date)"
echo "  Cases:             ${CASES}"
echo "  Active model:      FrankStarlingActiveStress (live, instantaneous)"
echo "  Ta_peak:           200 kPa"
...
for case_name in ${CASES}; do
    ...
    comment="frankstarling_l5_ta200_${case_name}_h5_6bt_fixed_unloaded_reference"
    ...
    if [ "${DRY_RUN}" = "1" ]; then
        ...
        jid=$(sbatch --parsable \
            --job-name="fs200_${case_name}" \
            --time=10:00:00 \
            --export=ALL,MESH=UKB,BPM=75,BEATS=6,POST_FULL=0,RUN_POSTPROCESS=1,CHAR_LENGTH=5.0,METRICS_SPACE=DG1,CIRCULATION_PARAMS="${json}",GEOMETRY_DIR="${GEOM_DIR}",GEOMETRY_FIELDS="${GEOM_FIELDS}",LOAD_UNLOADED_FROM="${SHARED_UNLOADED_FROM}",PRE_CIRC_BEATS=30,PRE_CIRC_MAX_BEATS=80,PRE_CIRC_CONVERGENCE_TOL=0.002,RESULTS_DIR_OVERRIDE="${result_dir}",COMMENT="${comment}",TA_PEAK_KPA=200.0,FS_PRELOAD_ONLY=0 \
            "${WORK_DIR}/sbatch/jobs/run_sim_and_post.sbatch")
    fi

    if [ -n "${jid}" ]; then
        JOB_IDS+=("${jid}")
        printf "%s\t%s\tfrankstarling_l5_ta200_submission_%s\tshared L5 geometry, shared unloaded reference, Frank-Starling (live) active stress at Ta_peak=200 kPa, strict canonical postprocessing; %s\t%s\t%s\t%s\t%s\t%s\n" \
